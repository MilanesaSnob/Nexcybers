...
El sitio es dark-first con variante light (`[data-theme="light"]` en `<html>`), y hay una tabla completa de hex en **`Nexcybers Color Palette.md`** (raíz del proyecto) — no duplicar esos valores acá, consultarla directo. Resumen de qué representa cada rol:

| Rol | Variable | Se usa para |
|---|---|---|
| Fondo de página | `--bg` | Body y bandas "theme". |
| Fondos alternos | `--bg-1`, `--bg-2` | Cards y superficies elevadas sobre el fondo de página. |
| Panel | `--panel`, `--panel-2` | Superficie de card en estado hover/activo. |
| Borde | `--line` (sutil) / `--line-2` (marcado) | Separadores y contornos de card. |
| Texto | `--fg` (principal), `--fg-soft` (secundario/bajadas), `--fg-mute` (terciario/timestamps) | Jerarquía de texto. |
| Acento de marca | `--gold` (+ `--gold-soft` hover) | CTA, eyebrows, links activos, foco. **Ojo:** el nombre de variable es histórico, el color real es celeste (`#00bffe` dark / `#00648c` light — AA sobre su fondo). |
| Celeste de banda | `--gold-bg` | Fondo grande de banda celeste (fijo `#0091c9`, no cambia por theme). Ver guía de "Nueva sección". |
| Dato técnico | `--blue` / `--cyan` | Gráficos, visualizaciones, chips de "activo". |
| Glow | `--gold-glow`, `--blue-glow` | Sombras/resplandores radiales detrás de cards y bandas CTA. |

Estados fijos: crítico, alto y activo mantienen su fondo (`rgba(244,46,46,.16)`, `rgba(244,179,46,.16)`, `rgba(56,225,255,.14)`) en los dos themes, pero **la tinta del texto cambia por theme para cumplir AA**: en dark `#ff6b6b` / `--gold-soft` / `--cyan`; en light `#a81b22` / `#8a4a07` / `#0f5f87`. Lo mismo con el estado "a recertificar" de product: `--warn-ink` es `#ff8f6b` en dark y `#a63a16` en light — nunca usar el coral claro sobre fondo claro.

**Regla de contraste:** texto sobre banda celeste (`--gold-bg`) va siempre en blanco o negro puro (`#fff`/`#000`), nunca en `--fg`/`--fg-soft` del theme — son los únicos dos casos en el sitio donde el color de texto se fija a mano en vez de usar variable, porque el fondo celeste es igual en los dos themes y el texto necesita el contraste correcto igual.

**Todo texto cumple AA (mínimo 4.5:1 texto de cuerpo, 3:1 texto de escala de título) contra su fondo real, salvo que se indique lo contrario explícitamente.** No usar un tono "suavizado" a mano (un hex intermedio tipo `#06283a` sobre celeste) para bajar el peso visual de un párrafo: si el texto necesita verse más liviano, se resuelve con `font-weight` o tamaño, nunca sacrificando contraste. Antes de fijar un color de texto nuevo sobre un fondo no cubierto por las variables del theme (bandas celeste, cards con fondo sólido), verificar el ratio contra ese fondo específico.
...
| `--radius` | 14px | Radio estándar de card (trust-card, contact-info-card, etc). |
