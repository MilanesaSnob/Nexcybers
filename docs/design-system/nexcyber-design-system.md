...
| Filter tabs (categoría) | `.res-tabs`, `.blog-tabs` | fondo `var(--bg-1)`, botón activo tiene color que depende de `data-cat` | — | Fila de botones que filtra la grilla de `.rcard`. El botón "All resources"/"All articles" (atributo `data-cat="all"`) usa `var(--gold)` cuando activo. Los botones de categoría específica (`.data-cat="ai"`, `"identity"`, `"phishing"`, `"privacy"`, `"risks"`, `"reg"`) usan su color `var(--tint)` en dark y `var(--tint-ink)` en light cuando están activos. En ambos casos: `font-weight:600` y `cursor:default` mientras están activos. | Filtrado de recursos y artículos por categoría (Resources, Blog). |
